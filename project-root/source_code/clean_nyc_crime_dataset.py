...
def drop_borough_column(df_crime: pd.DataFrame):
    df_crime.drop(columns="boro_nm",inplace=True) 
...
def assign_crime_group(row):
    ...
    standard_crimes = VIOLENT_CRIMES | PROPERTY_CRIMES | PUBLIC_ORDER_CRIMES
    if crime in standard_crimes:
...
def group_crimes_by_date(df_crime: pd.DataFrame) -> pd.DataFrame:
     # Returns a formatted DF with one set of crime incidents for each date instead of a date for each crime incident
     # 1/1/2023 | Larcency 
     # 1/1/2023 | Robbery
     # 1/1/2023 | Assault
     # ------ turns into 
     #1/1/2023 | total_property_crimes(1) | total_violent_crimes(2) 
    formatted_df_crime = (
    df_crime
    ...
    formatted_df_crime = formatted_df_crime.rename(columns={
    'property': 'total_property_crimes',
    'violent': 'total_violent_crimes',
    'public_order': 'total_public_order_crimes'
    })

    formatted_df_crime['total_crimes'] = (
    formatted_df_crime['total_property_crimes']
    + formatted_df_crime['total_violent_crimes']
    + formatted_df_crime['total_public_order_crimes']
    )
    return formatted_df_crime.reset_index()

def export_as_csv(df_crime: pd.DataFrame):
    df_crime.to_csv(CLEANED_NYC_CRIME_CSV, index=False)
    print(f"Saved cleaned weather data to: {CLEANED_NYC_CRIME_CSV}")
    print(df_crime.head())


def main():
    # Grabs csv file from RAW_SNAPSHOTS
    df_crime = fetch_csv(NYC_CRIME_CSV)
    # Standardizes date format 
    standardize_date_column(df_crime)
    # Merges dozens of crimes into 3 distinct categories (Property, Violent and Public Order)
    standardize_crime_column(df_crime)
    # Removes redundant borough column
    drop_borough_column(df_crime)
